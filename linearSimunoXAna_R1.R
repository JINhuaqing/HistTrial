...
get.results <- function(post.res){
    ...
    prb.wrong <- sapply(post.res, function(x) post.fn(x$sps.trts[, 2], 0.20))
...
# new resutls 
# betas = (1, 0)
H0.fil <- "./results/linearSimu_noX_1grpsame_nobeta1_H0.RData" 
load(H0.fil)
res.H0 <- get.results(post.res)
c.true <- eps.alig(res.H0$prbs[, 1], 0.05);c.true
c.wrong <- eps.alig(res.H0$prbs[, 2], 0.05);c.wrong

H1.fil <- "./results/linearSimu_noX_1grpsame_nobeta1_b03_H1.RData" 
load(H1.fil)
res.H1 <- get.results(post.res)
